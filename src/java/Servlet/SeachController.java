 ...
 * @author drako
 */
@WebServlet(name = "SeachControl", urlPatterns = {"/search"})
public class SeachController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet SeachControl</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet SeachControl at " + request.getContextPath() + "</h1>");
            String searchText = request.getParameter("text");
            ConnectDB cdb = new ConnectDB();
            cdb.getAllCategory();
            HashMap<String, Product> hmProduct = cdb.getAllProductBySearch(searchText);
